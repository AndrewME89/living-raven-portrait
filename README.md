...
Flight is a locked pair: `FLIGHT AWAY → paused first frame of FLIGHT RETURN → wait → FLIGHT RETURN`. No perched gesture can be selected while away, and only the completed return re-enters the normal scheduling queue. Flight Away enters the away state and pauses as soon as its WebGL render loop reaches `flightAwayCleanFrameSeconds`; the raw-video fallback enforces the same cutoff through `timeupdate`. This holds the first clean empty frame instead of exposing unwanted encoded tail frames while the hidden slot prepares Flight Return.
