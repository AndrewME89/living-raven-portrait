(function () {
  ...
  var cleanAwayFrameHandled = false;

  function enterAwayState() {
    ravenAway = true;
    portrait.classList.add('raven-away');
    still.classList.add('is-away');
  }

  function leaveAwayState() {
    ravenAway = false;
    portrait.classList.remove('raven-away');
    still.classList.remove('is-away');
  }

  function finishFlightAtCleanFrame() {
    if (currentClipName !== 'flightAway' || cleanAwayFrameHandled) return;
    if (!CONFIG.flightAwayCleanFrameSeconds || video.currentTime < CONFIG.flightAwayCleanFrameSeconds) return;
    cleanAwayFrameHandled = true;
    /* Enter AWAY before removing a single video pixel; normal can never leak through. */
    enterAwayState();
    video.pause();
    var finish = video.onended;
    if (finish) finish();
  }
  ...
  function playClip(name, onComplete) {
    ...
    function tryCandidate() {
      ...
      /* Raw-video fallback has no render loop, so timeupdate enforces the same clean cutoff. */
      video.ontimeupdate = finishFlightAtCleanFrame;
